...
export function mapProfile(raw: any): User {
  return {
    ...
    city: raw?.city,
    state: raw?.state,
    address: raw?.address || raw?.location || raw?.location_address || undefined,
    location: raw?.location || raw?.address || raw?.location_address || undefined,
    location_address: raw?.location_address || raw?.address || raw?.location || undefined,
